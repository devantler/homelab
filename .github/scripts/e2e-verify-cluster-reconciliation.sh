...
start_time=$(date +%s)
echo "Starting reconciliation of kustomization/infrastructure-secrets..."
...
kubectl -n flux-system wait kustomization/flux-system --for=condition=ready --timeout=5m || exit 1
