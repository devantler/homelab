...
kubectl -n flux-system wait kustomization/infrastructure --for=condition=ready --timeout=7m || exit 1
...
kubectl -n flux-system wait kustomization/apps --for=condition=ready --timeout=1m || exit 1
