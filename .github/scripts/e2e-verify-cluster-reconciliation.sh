start_time=$(date +%s)
kubectl -n flux-system wait kustomization/post-build --for=condition=ready --timeout=10m || exit 1
...
start_time=$(date +%s)
kubectl -n flux-system wait kustomization/infrastructure-configs --for=condition=ready --timeout=5m || exit 1
